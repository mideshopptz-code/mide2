# -*- coding: utf-8 -*-

print("Updating staff base.html with MIDE style...")

NEW_BASE = '''<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>{% block title %}Склад{% endblock %}</title>
<style>
:root{
  --black:#000;
  --white:#fff;
  --purple:#7c3aed;
  --purple-dark:#5b21a6;
  --purple-light:#a78bfa;
  --red:#ed1d36;
  --red-dark:#b91c1c;
  --gray:#f8f8fc;
  --gray-mid:#e5e5ea;
  --gray-text:#6b7280;
  --radius:16px;
  --radius-sm:10px;
  --shadow-sm:0 1px 3px rgba(0,0,0,.06);
  --shadow-md:0 4px 16px rgba(0,0,0,.08);
  --shadow-lg:0 12px 32px rgba(124,58,237,.15);
  --shadow-xl:0 24px 60px rgba(124,58,237,.25);
}
*{margin:0;padding:0;box-sizing:border-box}
body{
  font-family:-apple-system,'Segoe UI',Roboto,sans-serif;
  background:var(--gray);
  color:var(--black);
  line-height:1.6;
  -webkit-font-smoothing:antialiased;
}

/* ===================== NAV ===================== */
nav{
  background:rgba(255,255,255,.95);
  backdrop-filter:blur(14px);
  border-bottom:1px solid var(--gray-mid);
  position:sticky;top:0;z-index:100;
  display:flex;align-items:center;gap:6px;flex-wrap:wrap;
  padding:14px 24px;
  font-size:14px;
}
nav strong{
  font-size:22px;font-weight:900;
  letter-spacing:-1px;margin-right:18px;
  background:linear-gradient(135deg,var(--black),var(--purple));
  -webkit-background-clip:text;
  -webkit-text-fill-color:transparent;
  background-clip:text;
}
nav a{
  color:var(--black);text-decoration:none;
  padding:9px 16px;border-radius:var(--radius-sm);
  font-weight:600;transition:all .25s cubic-bezier(.4,0,.2,1);
}
nav a:hover{
  background:var(--purple);color:var(--white);
  transform:translateY(-2px);
  box-shadow:var(--shadow-md);
}
nav .role-badge{
  padding:3px 10px;border-radius:20px;
  font-size:11px;font-weight:800;color:#fff;
  text-transform:uppercase;letter-spacing:.5px;
}
.role-admin{background:var(--red)}
.role-senior_seller{background:#f39c12}
.role-mentor{background:#3498db}
.role-seller{background:#95a5a6}
.role-accountant{background:#9b59b6}

/* ===================== CONTAINER ===================== */
.container{max-width:1280px;margin:0 auto;padding:28px 24px}
h1{
  font-size:38px;font-weight:900;
  letter-spacing:-1.5px;margin-bottom:24px;
  background:linear-gradient(135deg,var(--black),var(--purple));
  -webkit-background-clip:text;
  -webkit-text-fill-color:transparent;
  background-clip:text;
}
h2{font-size:28px;font-weight:900;letter-spacing:-1px;margin-bottom:16px}
h3{font-size:19px;font-weight:800;margin-bottom:12px}

/* ===================== CARDS ===================== */
.card{
  background:var(--white);
  border:1px solid var(--gray-mid);
  border-radius:var(--radius);
  padding:24px;margin-bottom:20px;
  transition:all .4s cubic-bezier(.4,0,.2,1);
}
.card:hover{
  box-shadow:var(--shadow-lg);
  transform:translateY(-3px);
  border-color:var(--purple-light);
}

/* ===================== STATS ===================== */
.stats{
  display:grid;
  grid-template-columns:repeat(auto-fit,minmax(200px,1fr));
  gap:18px;margin-bottom:24px;
}
.stat-box{
  background:var(--white);
  padding:20px;border-radius:var(--radius);
  border:1px solid var(--gray-mid);
  transition:all .3s cubic-bezier(.4,0,.2,1);
}
.stat-box:hover{
  transform:translateY(-4px);
  box-shadow:var(--shadow-lg);
  border-color:var(--purple);
}
.stat-box h3{
  color:var(--gray-text);font-size:12px;
  text-transform:uppercase;letter-spacing:1px;
  margin-bottom:8px;font-weight:700;
}
.stat-box .value{
  font-size:32px;font-weight:900;
  color:var(--black);letter-spacing:-1px;
  line-height:1;
}

/* ===================== BUTTONS ===================== */
.btn{
  display:inline-flex;align-items:center;justify-content:center;gap:8px;
  padding:12px 24px;
  background:var(--purple);color:var(--white);
  border:none;border-radius:var(--radius-sm);
  font-size:14px;font-weight:700;
  cursor:pointer;text-decoration:none;
  transition:all .25s cubic-bezier(.4,0,.2,1);
  position:relative;overflow:hidden;
}
.btn::before{
  content:'';position:absolute;top:0;left:-100%;
  width:100%;height:100%;
  background:linear-gradient(90deg,transparent,rgba(255,255,255,.3),transparent);
  transition:left .6s;
}
.btn:hover::before{left:100%}
.btn:hover{
  background:var(--purple-dark);
  transform:translateY(-2px);
  box-shadow:var(--shadow-lg);
}
.btn:active{transform:translateY(0)}
.btn-success{background:#27ae60}
.btn-success:hover{background:#1e8449}
.btn-danger{background:var(--red)}
.btn-danger:hover{background:var(--red-dark)}
.btn-warning{background:#f39c12}
.btn-warning:hover{background:#d68910}
.btn-sm{padding:8px 14px;font-size:13px}

/* ===================== TABLE ===================== */
table{
  width:100%;border-collapse:collapse;
  background:var(--white);border-radius:var(--radius-sm);
  overflow:hidden;
}
th,td{
  padding:14px 16px;text-align:left;
  border-bottom:1px solid var(--gray-mid);
}
th{
  background:var(--gray);
  font-size:12px;text-transform:uppercase;
  letter-spacing:1px;color:var(--gray-text);
  font-weight:700;
}
tr{transition:background .2s}
tr:hover{background:var(--gray)}
tr:last-child td{border-bottom:none}

/* ===================== FORMS ===================== */
input,select,textarea{
  width:100%;padding:13px 16px;
  border:2px solid var(--gray-mid);
  border-radius:var(--radius-sm);
  font-size:15px;margin-bottom:14px;
  font-family:inherit;
  transition:all .25s cubic-bezier(.4,0,.2,1);
  background:var(--white);
}
input:focus,select:focus,textarea:focus{
  outline:none;
  border-color:var(--purple);
  box-shadow:0 0 0 4px rgba(124,58,237,.12);
  transform:translateY(-1px);
}
label{
  display:block;font-size:13px;font-weight:700;
  color:var(--black);margin-bottom:6px;
}

/* ===================== FLASH ===================== */
.flash{
  padding:14px 20px;border-radius:var(--radius-sm);
  margin-bottom:18px;font-weight:600;
  animation:slideIn .4s cubic-bezier(.4,0,.2,1);
}
.flash.success{background:#dcfce7;color:#166534;border-left:4px solid #22c55e}
.flash.error{background:#fee2e2;color:#991b1b;border-left:4px solid var(--red)}

/* ===================== TOAST ===================== */
#toast{position:fixed;top:20px;right:20px;z-index:9999}
.toast{
  background:var(--black);color:var(--white);
  padding:16px 20px;border-radius:var(--radius-sm);
  margin-bottom:10px;
  box-shadow:var(--shadow-xl);
  cursor:pointer;min-width:280px;
  border-left:4px solid var(--purple);
  animation:slideIn .4s cubic-bezier(.4,0,.2,1);
}

/* ===================== ANIMATIONS ===================== */
@keyframes slideIn{
  from{opacity:0;transform:translateX(20px)}
  to{opacity:1;transform:translateX(0)}
}
@keyframes fadeIn{
  from{opacity:0;transform:scale(.98)}
  to{opacity:1;transform:scale(1)}
}
@keyframes pulse{
  0%,100%{box-shadow:0 0 0 0 rgba(124,58,237,.7)}
  50%{box-shadow:0 0 0 12px rgba(124,58,237,0)}
}

/* ===================== MOBILE ===================== */
@media (max-width:768px){
  nav{padding:12px 16px;font-size:13px;gap:4px}
  nav strong{font-size:18px;width:100%;margin-bottom:6px}
  nav a{padding:7px 12px}
  .container{padding:20px 12px}
  h1{font-size:26px}
  .stats{grid-template-columns:1fr 1fr}
  .stat-box .value{font-size:24px}
  table{font-size:13px}
  th,td{padding:10px 8px}
}
</style>
</head>
<body>
<nav>
  <strong>Склад</strong>
  {% if user %}
    {% if user.role == 'accountant' %}
      <a href="/finance">💰 Финансы</a>
      <a href="/finance/transactions?type=income">📥 Поступления</a>
      <a href="/finance/transactions?type=expense">📤 Заявки</a>
      <a href="/admin/bank-accounts">💳 Счета</a>
      <a href="/notifications">🔔 Уведомления</a>
    {% else %}
      <a href="/">📦 Товары</a>
      <a href="/sales">💰 Продажи</a>
      <a href="/orders">📋 Заказы</a>
      <a href="/defects">🚨 Дефекты</a>
      <a href="/transfers">🔄 Переводы</a>
      {% if user.role in ('admin','senior_seller','mentor') %}
        <a href="/team">👥 Команда</a>
        <a href="/revisions">📝 Ревизии</a>
      {% endif %}
      {% if user.role == 'admin' %}
        <a href="/staff">👤 Сотрудники</a>
        <a href="/missions">🎯 Миссии</a>
      {% endif %}
      <a href="/dashboard">📊 Панель</a>
      {% if user.role in ('admin','senior_seller','mentor') %}
        <a href="/kpi">🏆 KPI</a>
      {% endif %}
      {% if user.role == 'admin' %}
        <a href="/zones">🗺️ Зоны</a>
      {% endif %}
      {% if user.role in ('admin', 'accountant') %}
        <a href="/finance">💰 Финансы</a>
      {% endif %}
      <a href="/profile">👤 Профиль</a>
      <a href="/notifications">🔔 Уведомления</a>
    {% endif %}
    <span style="margin-left:auto;font-size:13px;display:flex;align-items:center;gap:8px">
      {{ user.full_name or user.username }}
      <span class="role-badge role-{{ user.role }}">{{ user.role }}</span>
    </span>
    <a href="/logout">Выход</a>
  {% endif %}
</nav>
<div class="container">
  {% with messages = get_flashed_messages(with_categories=true) %}
  {% for cat,msg in messages %}
  <div class="flash {{cat}}">{{msg}}</div>
  {% endfor %}
  {% endwith %}
  {% block content %}{% endblock %}
</div>
<div id="toast"></div>
<script src="https://cdn.socket.io/4.7.2/socket.io.min.js"></script>
<script>
var socket = io();
socket.on('staff_notification', function(d){ showToast(d.title, d.body); });
function showToast(title, body){
    var box = document.getElementById('toast');
    var el = document.createElement('div');
    el.className = 'toast';
    el.innerHTML = '<b>'+title+'</b><br><span style="font-size:13px;opacity:.9">'+(body||'')+'</span>';
    el.onclick = function(){ el.remove(); };
    box.appendChild(el);
    setTimeout(function(){ el.remove(); }, 8000);
}
</script>
{% block scripts %}{% endblock %}
</body>
</html>'''

with open("templates/base.html", "w", encoding="utf-8") as f:
    f.write(NEW_BASE)
print("OK: base.html styled with MIDE")

# Проверка
c = open("templates/base.html", encoding="utf-8").read()
print()
print("Check:")
print("  size:", len(c))
print("  purple:", "#7c3aed" in c)
print("  red:", "#ed1d36" in c)
print("  has nav:", "<nav>" in c)
print()
print("=" * 50)
print("DONE!")
print("=" * 50)
print()
print("Теперь панель сотрудника выглядит как MIDE:")
print("  • Белое меню с blur-эффектом")
print("  • Фиолетовые кнопки с блеском")
print("  • Красивые карточки с тенями")
print("  • Плавные анимации")
print("  • Градиентные заголовки")
print()
print("Restart server + Ctrl+F5 in browser.")